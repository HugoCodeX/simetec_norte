...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      await fs.promises.access(fontRegular);
      await fs.promises.access(fontBold);
      await fs.promises.access(arialRegular);
      await fs.promises.access(arialBold);
    } catch (e) {
      console.error('Error: Fuentes no encontradas:', e);
      return NextResponse.json({ error: 'Error: Fuentes no disponibles' }, { status: 500 });
    }
      ...
      // Posición ajustada hacia la derecha para firma y timbre
       ...
       if (fs.existsSync(timbrePath)) {
         try {
           ...
           console.log('Error loading timbre:', error);
         }
       }
    ...
    return new NextResponse(new Uint8Array(pdfBuffer), {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="notificacion_defectos_${datosNotificacion.fechaNotificacion || new Date().toISOString().split('T')[0]}.pdf"`,
      },
    });
